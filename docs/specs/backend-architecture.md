...
```
Balenthiran.Habits/
├── Balenthiran.Habits.WebApi         # Entry point: routes, DI, OpenAPI, middleware
...
├── Balenthiran.Habits.Services       # Business logic implementations
│   └── Mapper.cs               # EntityModel ↔ DomainModel mappings
├── Balenthiran.Habits.Abstractions   # Interfaces only — no implementations
...
├── Balenthiran.Habits.EntityModels   # Database entity classes (anemic POCOs)
├── Balenthiran.Habits.DomainModels   # Rich business-layer objects
└── Balenthiran.Habits.DataModels     # Request/Response DTOs (public API contract)
```
