...
.NET Backend  (dotnet build -c Debug)
  └── Microsoft.Extensions.ApiDescription.Server  (GetDocument.Insider, in-process)
        └── backend/Balenthiran.Habits.WebApi/openapi.json  (committed schema)
...
```js
const config = {
  schemaFile: '../backend/Balenthiran.Habits.WebApi/openapi.json',
...
```csharp
// ✅ OpenAPI knows the 200 body is HabitView and the 404 has no body
...
group.MapGet("/{id:int}", async (int id, IHabitService habits) =>
    await habits.GetAsync(id) is { } h ? Results.Ok(h) : Results.NotFound());
```
...
- The response type `T` must be a **concrete record** (`HabitView`), not a service view
  **interface** (`IHabitView`). Services return interfaces; the handler maps to the
  concrete record via `IMapper` (see `WebApi/Mapper.cs`) so the wire contract — and the
  generated TypeScript type — is a real, named shape.
- A shape the service does not return directly (e.g. the status payload) gets a concrete
  `*Response` record the route assembles.
...
1. **Refresh the schema** with a Debug backend build (regenerates the committed
   ...
   ```bash
   cd backend
   dotnet build Balenthiran.Habits.WebApi -c Debug
   ```
...
1. Add route in `backend/Balenthiran.Habits.WebApi/Routes/*.cs`
2. Ensure the route is registered in `Program.cs` within the `.WithOpenApi()` chain
3. Rebuild the backend in Debug (`dotnet build Balenthiran.Habits.WebApi -c Debug`) to refresh `openapi.json`
