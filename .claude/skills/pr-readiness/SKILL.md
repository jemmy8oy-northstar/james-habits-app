...
- [ ] No concrete types (classes, records, structs) defined in `Balenthiran.Habits.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `Balenthiran.Habits.Abstractions.csproj` has **zero** `<ProjectReference>` entries to projects with concrete types
...
- [ ] Domain models are in `Balenthiran.Habits.DomainModels`, data models in `Balenthiran.Habits.DataModels`
